...
void Application::evolution()
{
    ...
    while(newPopulation_.getSize() != actualPopulation_.getSize())
    {
        ...
        if(ret.second == true || iterations > 1000000)
        {
            emit getAssessed(i);
...
{
    if((inputImage_.rows != referenceImage_.rows) || (inputImage_.cols != referenceImage_.cols))
    {
        emit throwException(string("Images size is not the same"));
        terminate();
    }
    try {
    stats_.clear();
...
//        actualPopulation_.savePopulation(generationNumber_, katalog_);
    ...
    {
        emit throwException(ex);
    }
    catch (std::exception & e)
    {
        emit throwException(string(e.what()));
    }
    catch (const char *exc)
    {
        emit throwException(string(exc));
    }
}
