    ...
    isInputImage_(false), isReferenceImage_(false), isOptionSetup_(false)
{
    ...
    QObject::connect(&app_, SIGNAL(throwException(std::string)), this, SLOT(catchException(std::string)));
...
void MainWindow::catchException(string exc)
{
    app_.terminate();
    QErrorMessage * message = new QErrorMessage(this);
    QString e = QString::fromStdString(exc);
